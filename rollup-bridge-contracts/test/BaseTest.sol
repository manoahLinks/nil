...
contract BaseTest is Test {
  ...
  function updateStateWithTestData(BatchData memory batchData) public {
    for (uint256 i = 0; i < batchData.batches.length; i++) {
      ...
      INilRollup.PublicDataInfo memory publicDataInfo = INilRollup.PublicDataInfo({
        l2Tol1Root: batchDataItem.l2Tol1Root
      });
  ...
  function transformBatchInfoToBatchData(BatchInfo memory batchInfo) public view returns (BatchData memory) {
    ...
    for (uint256 i = 0; i < batchInfo.batches.length; i++) {
      ...
      console.log("inside transformBatchInfoToBatchData before doing for l2Tol1Root");

      bytes32 l2Tol1Root = parseBytes32(infoItem.l2Tol1Root);
  ...
  function parseBatchInfo() public view returns (BatchInfo memory) {
    ...
    console.log("inside parseBatchInfo");
    BatchInfo memory batchInfo = abi.decode(blobDataBytes, (BatchInfo));
    console.log("after parseBatchInfo");
    return batchInfo;
  }
