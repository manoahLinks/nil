...
func (bs *BlockStorage) validateBatchSequencing(tx db.RoTx, batch *scTypes.BlockBatch) error {
	currentRefs, err := bs.ops.getLatestFetched(tx)
	if err != nil {
		return err
	}

	for shard, batchParentRef := range batch.ParentRefs() {
		currentRef := currentRefs.TryGet(shard)

		if currentRef == nil || batchParentRef.Equals(currentRef) {
			continue
		}

		return fmt.Errorf(
			"%w, id=%s: parent ref %s does not match current ref %s",
			scTypes.ErrBatchMismatch, batch.Id, batchParentRef, currentRef,
		)
	}
