...
func (agg *aggregator) runIteration(ctx context.Context) {
	err := agg.processAndHandleErr(ctx)
	if err != nil {
		agg.metrics.RecordError(ctx, agg.Name())
	}
}

// processAndHandleErr fetches and processes new blocks for all shards.
// It handles the overall flow of block synchronization and proof creation.
func (agg *aggregator) processAndHandleErr(ctx context.Context) error {
...
func (agg *aggregator) getLatestBlockRef(ctx context.Context) (*types.BlockRef, error) {
	...
	blockRef := types.RefToBlock(block)
	return &blockRef, nil
}
...
func (agg *aggregator) getStartingBlockRef(ctx context.Context) (*types.BlockRef, error) {
	...
	if mainRef := latestFetched.TryGetMain(); mainRef != nil {
		if err := agg.requireBlockExistsInChain(ctx, *mainRef); err != nil {
			return nil, err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("error fetching main block with hash=%s: %w", *latestProvedRoot, err)
	}
	if rpcBlock == nil {
		return nil, fmt.Errorf(
			"%w: block with hash=%s (latest proved state root) not found in chain",
			types.ErrBlockMismatch, *latestProvedRoot,
		)
	}

	blockRef := types.RefToBlock(rpcBlock)
	return &blockRef, nil
}

func (agg *aggregator) requireBlockExistsInChain(ctx context.Context, blockRef types.BlockRef) error {
	rpcBlock, err := agg.rpcClient.GetBlock(ctx, blockRef.Shard, blockRef.Hash, false)
	if err != nil {
		return fmt.Errorf("error fetching block from RPC, %s: %w", blockRef, err)
	}
	if rpcBlock == nil {
		return fmt.Errorf("%w, block not found in chain, %s", types.ErrBlockMismatch, blockRef)
	}
	if rpcBlock.Number != blockRef.Number {
		return fmt.Errorf(
			"%w, block number mismatch, got %d, expected %s", types.ErrBlockMismatch, rpcBlock.Number, blockRef,
		)
	}
	return nil
}
