...
func (es *ExecutionState) HandleTransaction(
	...
	if res.Error != nil && !responseWasSent {
		...
		if txn.IsBounce() {
			logger.Error().
				Err(res.Error).
				Msg("VM returns error during bounce transaction processing")
		} else {
...
func (es *ExecutionState) handleDeployTransaction(
	_ context.Context,
	transaction *types.Transaction,
) (res *ExecutionResult) {
	addr := transaction.To
...
func (es *ExecutionState) AddToken(
	addr types.Address,
...
func (es *ExecutionState) SubToken(
	addr types.Address,
	tokenId types.TokenId,
	amount types.Value,
) error {
